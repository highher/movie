...
<c:choose>
	...
	<div class = "view-main">
		...
		<div class="view-main2" id="info" >
			...
			<div class="flex-box" style="margin-top:40px">
	            <div class ="flex-box-item flex-box-item-selected" onclick='location.href="movie?command=view&movie_id=${movie.id}"'>
	               상세정보
	            </div>
	            <div class ="flex-box-item not" onclick='location.href="movie?command=view_comment&movie_id=${movie.id}#info"'>
	               관람평가
	            </div>
			...
			<div class= "view-section-1" style="padding-bottom:25px;border-bottom:1px solid lightgray">
				<h1>${movie.catchprise}</h1>
				<p>${movie.content}
			</div>
			
		</div>
		<div class="view-main2">
			<div class ="view-section-1" style="padding-top:10px; padding-bottom:10px;">
			<p><span style = "margin:0px 10px 0px 10px;">감독: ${movie.director}</span ><span style = "margin:0px 10px 0px 10px;">장르: ${movie.genre}</span><span style = "margin:0px 10px 0px 10px;">상영시간: ${movie.time}분</span>
			<br/><span style = "margin:0px 10px 0px 10px;">출연진: ${movie.actor}</span></p>
			</div>
		</div>
		<div class="moviecinfo-layout" style="margin-top:30px">
	        <div class="moviecinfo-layout-items">
	            <ul>
	                <li>관람포인트</li>
	    			<li><canvas style="margin:0px auto; margin-top:-10px;" id="Chart" width="250" height="250"></canvas></li>     	
	            </ul>
	        </div>
	         ...
	         const data = {
	        		    labels: ['스토리','OST','연출','배우','영상미'],
	        		    datasets: [{
	        		      label: '',
	        		    
	        		      data: [<c:out value= "${commentData.story}"/>,<c:out value= "${commentData.ost}"/>,
		        		    	  <c:out value="${commentData.directing}"/>,<c:out value="${commentData.acting}"/>,
		        		    	  <c:out value="${commentData.visual}"/>],
	        		      fill: false,
	        		      borderColor: '#037B94',
	        		    }]
	        		  };
	        ...
	        <div class="moviecinfo-layout-items">
	            <ul>
	                <li>실관람평점</li>
	                <li><h1 style="background-color:#037B94; border-radius:50%;color:white; line-height:100px">${movie.score}</h1></li>
	                <li>예매율</li>
	                <li><h1 style = "color:#6543B1">
               			<c:choose>
						        <c:when test="${movie.total eq 0}">
						        <c:out value="-">
						        </c:out>
	        ...
	        <div class="moviecinfo-layout-items">
	            <ul>
	                <li>누적관객수</li>
	                <li style="font-size:40px; line-height: 100px; font-weight: 600;">
             				<c:choose>
						        <c:when test="${movie.view eq 0}">
						        <c:out value="-">
						        </c:out>
	                ...
	                <li>순위</li>
	                <li style="font-size:40px; line-height: 100px; font-weight: 600; color:#6f00cc;">
	                        <c:choose>
						        <c:when test="${movie.view eq 0}">
						        <c:out value="-">
						        </c:out>
						        ...
						        <c:otherwise>
						      	<fmt:formatNumber value="${movie.ranking}" pattern="#,###"/><c:out value="위"></c:out>
						        </c:otherwise>
							</c:choose>
	                </li>
	            </ul>
		...
		<div class = "view-main2" style="margin-bottom:20px">
    ...
    <div class="moviecomment-layout">
        ...
        <table class = "moviecomment-table" style = "margin-top:10px; margin-bottom:10px;">
        ...
        <div style="text-align: end; margin-bottom: 40px; cursor: pointer;">
        	<a style="color:black" href="movie?command=view_comment&movie_id=${movie.id}#info">관람평가 더보기</a>
        </div>
    </div>
